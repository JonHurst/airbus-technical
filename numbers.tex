...
\item[$\bullet$] \textbf{Minimum required equipment:}\\ 1 FMS; 1 GPS; 2 IRS;
  1 MCDU; 1 FD; 1 PFD (on PF side); 2 ND (tmp display OK); 2 FCU channels. \extref{PRO.SPO.51}
...
\item[$\bullet$] Modifications to FMC database procedure prohibited with
  ...
  \item[$\bullet$] Fly a stabilised approach if using FPA. Set and pull FPA
    0.3nm before descent point. \extref{EOMB 2.3.18.3.4}
...
%% ** ILS approach
\mysec{ILS approach}{
\item Standard coverage \extref{LIDO GEN.NAV.3.2}:
...
\item Absolute minimas (DH/TDZ RVR) \extref{EOMA 8.1.3.9.3}:
...
\item[$\bullet$] All passenger PEDS must be turned off. \extref{EOMA 8.3.23.2}
}
...
\item[$\bullet$] Extra calls: \extref{EOMB 2.3.18.3.1.2}

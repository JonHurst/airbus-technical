% -*- eval: (orgstruct-mode); orgstruct-heading-prefix-regexp: "%% "; -*-
...
\item Takeoff or landing is not permitted with: \ref{EOMB 2.1}
  \begin{compactitem}
  \item Wet ice
  ...
  \item Ldg Dispatch, wet ice and layered contaminants \ref{EOMB 2.1}\dotfill 2 alternates rqd
  \item Minimum cleared width \ref{EOMB 2.1} \dotfill 30m (check
    snowbanks: EOMB 4.6.8)
    \iitem $\leq$45m does not need to be
    treated as a narrow runway for V$_1$ etc.
...
%% ** Other wind limits
  \mysec{Other wind limits}{
    \item LEAP-1A starting max crosswind \ref{FCOM LIM.EBG}\dotfill 45kt
\item Passenger door operation \ref{FCOM LIM.AG.OPS}\dotfill 65kt
\item Cargo door operation \ref{FCOM LIM.AG.OPS}\dotfill 40kt (50kt w/caveats)
\item Cargo door closed before \ref{FCOM LIM.AG.OPS}\dotfill 65kt
}
...
\item Max slope \ref{FCOM LIM.AG.OPS} \dotfill $\pm$2\%
\item Max runway altitude \ref{FCOM LIM.AG.OPS} \dotfill 9200ft
\item Nominal runway width \ref{FCOM LIM.AG.OPS} \dotfill 45m
\item Minimum runway width \ref{FCOM LIM.AG.OPS} \dotfill 30m
...
\mysec{Loading}{
  ...
  \item\textbf{A321 Protocol:}\ref{GHM 5.4.1.2}
...
\iitem Positive LMC < 250kg, $\Delta$CG < 2\% \dotfill Subtract 1 from FLEX\footnotemark
...
\item A320 Forward CG \ref{EOMB 4.9.1.1} \dotfill CG < 27\%MAC
}
\footnotetext{FLEX temperature must always be greater than OAT and TREF (A319:ISA+30; A320:ISA+29; A321:ISA+15)}
...
\item Oil
  \iitem CFM56 min quantity\ref{EOMB 2.3.4.5}\dotfill 9.5qt+0.5qt/hr
  \iitem LEAP-1A min quantity\ref{EOMB 2.3.4.5}\dotfill 10.6qt+0.45qt/hr\footnotemark
...
\mysec{Electrical}{
\item Prelim prep min battery voltage \ref{EOMB 2.3.4.2} \dotfill $>$25.5V
\item Battery check \ref{EOMB 2.3.6.2} \dotfill charge current<60A within 10s
}
...
\item Ram air max diff \ref{FCOM DSC.12.10.20}\dotfill 1psi
}
...
\item Ground icing conditions \ref{EOMB 2.3.9}\dotfill OAT$\le$10°C + vis moist | gnd contam
\item Flight icing conditions \ref{EOMB 2.3.9}\dotfill TAT$\le$10°C + vis moist
\item Eng anti-ice not rqd \ref{EOMB 2.3.13}\dotfill climb/cruise,
  SAT<-40°C, no CBs
...
%% ** Gear limitations
\mysec{Gear}{
\item Extend \ref{LIM.AG.SPD}\dotfill 250kt
\item Retract \ref{LIM.AG.SPD}\dotfill 220kt
\item Extended \ref{LIM.AG.SPD}\dotfill 280kt/M.67
\item Max taxi speed, single tyre deflated \ref{LIM.LG}\dotfill 7kt
\item Max taxi speed, both tyres deflated \ref{LIM.LG}\dotfill 3kt
\item Max steering angle, both tyres deflated \ref{LIM.LG}\dotfill 30°
\item Max brake temp for takeoff \ref{LIM.LG}\dotfill 300°
}
...
\item Ops with ``Low Oil Level'' ECAM \ref{FCOM.PRO.ABN.ADV} \dotfill 10hrs
}

%% ** Navigation limitations
\mysec{Navigation}{
\item Max IRS latitudes \ref{FCOM LIM.NAV}\dotfill 73°N,60°S\footnotemark
\item Altimeter tolerances \ref{EOMB 2.3.6.11}
...
\iitem Cat II approach, manual landing \ref{FCOM LIM.AFS.20} \dotfill 80ft agl
...
\item Runway lighting \ref{LIDO GEN-1.3.1.6}
...
\item Alternate ranges \ref{EOMB 5.1}
...
\item Avoid reducing below V$_\mathrm{LS}$ (including GPWS/Windshear) \ref{FCOM PRO.ABN.ENG}
\item Autoland capability \ref{FCOM LIM.AFS.20}\dotfill Cat III single (A320CEO $\Rightarrow$ Conf
  Full)
\item Available NPA Autopilot modes \ref{FCOM LIM.AFS.10}
  \iitem A320/A321 \dotfill All
  \iitem A319 \dotfill LOC/VS, LOC/FPA, HDG/VS, TRK/FPA\footnotemark
\item Do not extend full flaps until established on final descent. \ref{FCOM PRO.ABN.ENG}
\item Use Conf 3 if a level off is required. \ref{FCOM PRO.ABN.ENG}
\item Check QRH ABN.ENG.OEI Circling Approach if circling required
\item Sharklet automatic rollout \ref{FCOM LIM.AFS} \dotfill Idle reverse only
}
...
\item Rapid decompression \ref{EOMB 3.80.2}:
...
%% ** General
\mysec{General}{
\item Procedure turn \ref{LIDO GEN.1.5.5.6.1.2} \dotfill 75 seconds from start of 45° turn
}
...
\item[$\bullet$] Protected circling radius \ref{LIDO GEN.1.5.5.6.2.5} \dotfill
...
\mysec[\secref{EOMB 2.3.18.3.2}]{RNAV or Non-Precision overlay approach}{
\item[$\bullet$] \textbf{Minimum required equipment:}\\ 1 FMS; 1 GPS; 2 IRS;
  1 MCDU; 1 FD; 1 PFD (on PF side); 2 ND (tmp display OK); 2 FCU channels. \ref{PRO.SPO.51}
  \item[$\bullet$] Max acceptable difference between altimeters \dotfill 100ft
\item[$\bullet$] RNP(AR)\footnotemark approaches are not yet authorised. \ref{EOMA 8.3.2.6}
...
\item FALS/IALS/BALS/NALS \dotfill LIDO GEN 1.7.7.10.6.2
}\label{ils}
